...
//
//		//types of mutexes 
//		cout << "func " << "   " << std::this_thread::get_id() << "   " << m_nvalue++ << endl;
//		//mymutex.unlock();
//
//		//cout << "from t1" << i << endl;
...
/*
try_lock : 
	if the lock is aquired by other thread then the second thread will return immediatley if it couldn't aquire the lock 

try_lock_for : 
	Its the same as above expect that we can specify how much time the thread 2 can wait to get the lock. If its
	 not able to get the lock in that time it returns 

try_lock_until :
	This is an extended version of the try_lock_for. when we use try_lock_for the thread waits until that time where as try_lock_until 
	allows thread to wait until that given time(absolute time)
	For example : The thread 2 will try to get lock by current time + 10 seconds - if it doesn't then it goes back
...
int value = 0;
void Increment(int thread)
{
	if (timedmutex.try_lock())
	{
		cout << "Lock aquried by thread " << thread << endl;
	...
	{
		cout << "thread " << thread << " couldn't get lock" << endl;
	}
...
/*
recursive_mutex is used when there is a situation where we have to lock the same mutex by the same thread 
multiple times  before its unlocked 
unlock mutex should be called same number of times the lock mutex has been called else program might have undefined behaviour 

*/
//std::mutex m1; //program crashes as we are locking the same mutex in reset function without unlocking it in process function
// 
// 
//std::recursive_mutex m1; // Program runs fine
//int _variable = 0;
//
//void reset()
//{
...
/*
Shared_mutex provides provides ability to provide shared_access across mutex i.e. different threads 
can be owners of same mutex at same point of time. It supports the following 3 methods
lock_shared
try_lock_shared
unlock_shared
...
If one thread has acquired the exclusive lock (through lock, try_lock), no other threads can acquire the lock (including the shared).
	Behaviour is like normal mutex
...
//	std::this_thread::sleep_for(std::chrono::seconds(1));
//	_value++;
//	cout << "value while incrementing " << _value << endl;
//	shmutex.unlock();
//}
//
////since multiple threads have to read the value at the same point of time safely - shared_mutex can be used here 
//void readvalue()
//{
//	shmutex.lock_shared();
//	std::this_thread::sleep_for(std::chrono::seconds(1));
//	cout <<"value while reading is " << _value << endl;
...
//	for (int i = 0; i < 5; ++i)
//	{
//		tWrite[i] = std::thread(write);
//	}
//
//	std::thread tRead[5];
//	for (int i = 0; i < 5; ++i)
//	{
//		tRead[i] = std::thread(readvalue);
//	}
//
//	for (auto& th : tWrite)
//		th.join();
...
//	balance += money;
//	cv.notify_all(); //This notifies the thread tdrawmoney that it has finished its job 
...
//
//	std::thread tdrawmoney(ReduceMoney, 100);
//
//	taddmoney.join();
//	tdrawmoney.join();
//}
...
//void ProcessingData()
//{
//	std::cout << "Data is processed" << endl;
//}
//
//
//void waitingforData()
//{
//	std::cout << "waiting for data preparation" << endl;
//	std::unique_lock<std::mutex> ulock(_mutex);
//	_cv.wait(ulock, [] {return bReady.load(); });
//	ProcessingData();
//}
//
//
//void PrepareData()
//{
//	std::cout << "Data is being prepared" << endl;
//	bReady = true;
//	_cv.notify_one();
//}
//
...
//	std::future<int> res = std::async(std::launch::async, factorial, 3); //as factorial function returns int value we need to have a future object of int type <int>
//	cout << res.get() << endl; //we can fetch the value from res using a get method. This also waits until the child thread return a value 
...
//
//	std::future<int> res = std::async(std::launch::async | std::launch::deferred, factorial, std::ref(fut));
//
...
// 1.  we need not call either join or detach on teh thread and the compiler will automatically takes care of it
...
//
//private:
//	bool m_bConnectionalive = false;
//
//	void connect()
//	{
//		cout << "I am child thread " << std::this_thread::get_id() << endl;
//		std::this_thread::sleep_for(std::chrono::seconds(2));
...
//};
//
//std::shared_ptr<IConnection> Makeconnection()
//{
//	std::shared_ptr<IConnection> p = std::make_shared<Connection>();
//	return p;
//
//}
//
...
//	conn->ISAlive() ? (cout << "connection established " << endl) : (cout << "connection establishment failed " << endl);
//	cout << "I am main thread " << std::this_thread::get_id() << endl;
//}
